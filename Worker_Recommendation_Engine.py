...
class Worker_Recommendation_Engine:
    ...
    def matching_algorithm(self):
        ...
        for job_id in jobs:
            ...
            worker_matrix = np.zeros((num_workers, len(self.attribute_weights_array)))
            ...
            worker_weighted_matrix = worker_matrix * self.attribute_weights_array
            worker_score_vector = np.sum(worker_weighted_matrix, axis=1)

            # Pair each worker with their score
            worker_scores.update(dict(zip(workers, worker_score_vector)))

            # Sort the workers' indices based on their scores in descending order
            worker_sorted_indices = np.argsort(worker_score_vector)[::-1]
            sorted_workers = [workers[idx] for idx in worker_sorted_indices]
            worker_sorted_scores = worker_score_vector[worker_sorted_indices]
            ...
            #print()
            #for worker_id, score in zip(sorted_workers, worker_sorted_scores):
                #worker_list.append(worker_id)
            #print()

            worker_recommendations[job_id] = sorted_workers
        
        return worker_recommendations
...
wre = Worker_Recommendation_Engine()
db = wre.firebase()
wre.retrieve_organisation_data()
wre.retrieve_job_data()
wre.retrieve_worker_data()
wre.score_qualification()
wre.calculate_worker_age()
print(wre.matching_algorithm())